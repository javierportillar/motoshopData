...
| **R11** | Métricas ML F4-B no auditadas (Prophet MAPE 3540% + Classifier F1 0.9924) | ✅ **Resuelto** (Sesión 43) | F4-FIX1 corrigió métrica (WAPE primaria) + filtro elegibilidad SKU + split temporal + remover `stock_actual` del feature set. Métricas honestas finales: Prophet WAPE 864% (documentado como no-recomendado en ADR-0017), LightGBM WAPE 57%, Baseline WAPE 45.83% (champion 97.9% SKUs), Classifier F1 0.536 con split temporal limpio. Ver `v_model_evaluation_20260530_113116.md` + `v_classifier_stockout_20260530_113711.md`. |
| **R12** | F4-C usó FakeRepos en prod en lugar de validar contra Gold real | ✅ **Resuelto** (Sesión 43) | F4-FIX1 cambió DI a Real* repos cuando `env != "test"`. Verificado match PWA↔Databricks SQL: 6/6 campos forecast MOTS1297 + 10/10 SKUs alertas. Evidencia: `v_fix1_forecast_real.md` + `v_fix1_alertas_real.md`. |
| **R13** | R10 mitigación insuficiente: PWA no alerta al usuario datos stale | ✅ **Resuelto** (Sesión 43) | StaleDataBanner implementado: banner amarillo si `lag_hours > 24h`, banner rojo si health endpoint caído, oculto si fresco. 4/4 casos E2E Playwright. Evidencia: `v_fix1_stale_banner.md`. |
| **R14** | Prophet y LightGBM siguen en el pipeline pese a ser inservibles | ✅ **Resuelto** (F5-A, Sesión 45) | Archivados: `docs/archive/infra/run_forecast_prophet.py`, `run_forecast_lightgbm.py`, `docs/archive/gold/20_forecast_prophet.py`, `21_forecast_lightgbm.py`. `infra/create_gold_workflow.py` skip las tasks correspondientes. README explicativo en `docs/archive/infra/README.md`. Baseline queda como único modelo de forecasting de producción. |
| **R16** | API puede arrancar con `ENV=test` accidental en producción → usa FakeRepos silenciosamente | 🔴 Abierto · **trigger F6 hardening** (descubierto Sesión 45) | Durante deploy F5 a Windows, `.env` quedó con `ENV=test` por error. La API arrancó con `FakeAlertActionsRepo` y NO escribía a MySQL — detectado por coincidencia. No hay guardrail que bloquee. **Mitigación:** F6-A1 agregar startup check que falle si `settings.env == "test"` y host != `localhost`, o requerir flag explícito `ALLOW_TEST_ENV_IN_PROD=true`. **Trigger:** primer sprint F6. |
| **R15** | Password `FG28` re-propagada en `motoshop-app/api/users.yaml` (gitignored pero force-added) | 🟡 Aceptado · **diferido a F6 hardening** (decisión humana 2026-05-30) | Commit `cc04491` agregó users.yaml al tracking con `# Password compartida: FG28` en comentario; el archivo SÍ está en `motoshop-app/api/.gitignore` pero fue force-added. Extiende deuda R2 a un archivo nuevo. **Trigger:** F6 hardening — rotación FG28 + `git rm --cached users.yaml` + regenerar como gitignored. Decisión humana: dejar el archivo como está hasta F6 para no romper auth local que ya funciona. |
