...
DELETE FROM motoshop.gold._quality_runs
...
SELECT assert_true(
  (SELECT COUNT(*) FROM motoshop.gold._quality_runs WHERE severity = 'CRITICAL' AND DATE(timestamp) = CURRENT_DATE()) = 0,
  CONCAT('Quality gold encontró ', CAST((SELECT COUNT(*) FROM motoshop.gold._quality_runs WHERE severity = 'CRITICAL' AND DATE(timestamp) = CURRENT_DATE()) AS STRING), ' errores CRITICAL')
