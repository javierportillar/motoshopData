...
def test_entity_ref_is_server_resolved_and_tenant_authorized() -> None:
    ref = resolve_entity_ref(_context("inventory"), entity_type="product", entity_id="SKU-1",
                             label="Filtro", domain="inventory", route_key="product")
    assert ref.href == "/inventario/productos/SKU-1"
    with pytest.raises(PermissionError):
        resolve_entity_ref(_context("sales"), entity_type="product", entity_id="SKU-1",
                           label="Filtro", domain="inventory", route_key="product")
    with pytest.raises(ValueError):
        resolve_entity_ref(_context("inventory"), entity_type="product", entity_id="https://x",
                           label="Filtro", domain="inventory", route_key="product")
...
def test_duckdb_adapter_binds_tenant_and_limits_rows() -> None:
    ...
    rows, evidence = DuckDBSourceAdapter(
        _context("sales"), connection=Connection()
    ).read("sales", {})
